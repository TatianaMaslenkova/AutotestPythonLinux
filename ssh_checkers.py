...
def upload_files(host, user, passwd, local_path, remote_path, port=22):
    print(f"Загрузка файла {local_path} в каталог {remote_path}")
...
def download_files(host, user, passwd, remote_path, local_path, port=22):
    print(f"Скачивание файла {remote_path} в каталог {local_path}")
    transport = paramiko.Transport((host, port))
    transport.connect(None, username=user, password=passwd)
    sftp = paramiko.SFTPClient.from_transport(transport)
...
if __name__ == "__main__":
    # Скачивание файла со статистикой из user2 в папку проекта
    with open('config.yaml') as f:
        data = yaml.safe_load(f)
    download_files(data["ip"], data["user"], data["passwd"], "/home/user2/logfolder/stat.txt", "stat.txt")
